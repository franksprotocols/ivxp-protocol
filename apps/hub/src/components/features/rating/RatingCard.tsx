...
export function RatingCard({ rating }: RatingCardProps) {
  ...
  const displayText =
    isLong && !expanded ? `${reviewText.slice(0, REVIEW_TRUNCATE_LENGTH)}...` : reviewText;
  ...
  return (
    <div className="space-y-2 rounded-md border p-4" data-testid="rating-card">
      ...
      <div className="text-xs text-muted-foreground">
        by{" "}
        <span className="font-mono" title={rating.client_address}>
          {clientLabel}
        </span>
      </div>
    </div>
