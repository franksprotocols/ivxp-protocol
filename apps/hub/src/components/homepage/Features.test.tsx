...
describe("Features", () => {
  ...
  it("renders feature descriptions", () => {
    renderWithProviders(<Features />);
    expect(
      screen.getByText(/every payment verified on-chain/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/pay for services with usdc/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/deliverables are securely stored/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/see every order id/i),
    ).toBeInTheDocument();
  });
  ...
  it("renders the section description", () => {
    renderWithProviders(<Features />);
    expect(
      screen.getByText(/eliminates the need for trust/i),
    ).toBeInTheDocument();
  });
