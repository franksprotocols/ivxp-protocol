...
describe('SchemaGenerator.generate', () => {
  ...
  it('returns one MCPTool per service for multi-service catalog', () => {
    const tools = SchemaGenerator.generate(
      makeCatalog(['summarize', 'translate', 'ocr']),
    );
  ...
  it('sets service enum from all catalog service types', () => {
    const tools = SchemaGenerator.generate(
      makeCatalog(['summarize', 'translate']),
    );
    ...
    expect(tools[1].inputSchema.properties.service.enum).toEqual([
      'summarize',
      'translate',
    ]);
  });
