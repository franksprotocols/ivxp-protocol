...
For delivery requests, two signature profiles are used in current implementations:

1. **Strict profile (recommended)** using canonical rendering:
...
When strict profile is enabled, `nonce` prevents replay attacks even if the same `order_id` and `tx_hash` appear again. Providers should track seen nonces per order and reject duplicates.
`signed_message` must match provider verification logic for the selected profile.
...
```
1. message = provider-defined deterministic rendering of order/payment context
...
6. if strict profile enabled: assert nonce has not been seen before for this order_id
7. assert signed_message equals expected message built from request fields
...
**Mitigation:** Each delivery request includes a unique `order_id` and `timestamp`, with optional `nonce` in strict profile. The provider:

1. Tracks processed nonces per `order_id` when nonce is used
